...
/*
 * DisplayManyChoose.java
 * 
 * Created on 2009-2-26 下午03:33:04
 * 
...
public class DisplayManyChoose extends Display {
	...
	private void genarateInput(Page page){
		for (Item item: page.getItems()) {
			...
			for (AnswerOption answerOption: item.getAnswerOptions()) {
				String option  = answerOption.getContent().trim();
				option = option.replaceFirst("^<p>",  "");
				option = option.replaceFirst("</p>$", "");
				answerOption.setContent(option);
			}
		}
	}
