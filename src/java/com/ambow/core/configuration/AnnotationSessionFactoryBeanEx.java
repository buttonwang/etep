/**
 * 参考 http://www.javaeye.com/topic/198525
 */
...
public class AnnotationSessionFactoryBeanEx
  ...
  /**  
   * @see AnnotationSessionFactoryBeanEx#postProcessAnnotationConfiguration(org.hibernate.cfg.AnnotationConfiguration)  
   */  
  @Override  
    ...
    try {   
      for (String regex : excludedClassesRegexPatterns) { //Test each patterns.   
        ...
        if (matcher.matches(className, pattern)) {   
          logger.debug("class "+className+", matches [{}], so it is excluded."+pattern.getPattern());   
          return true;   
        }   
      }   
