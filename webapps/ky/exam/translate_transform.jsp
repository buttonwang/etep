                            ...
                            <tr>
                              ...
                              <td colspan="3">
                                <table width="100%" border="0" cellspacing="0" cellpadding="0">
								  ...
								  <c:forEach items="${item.answerOptions}" var="answerOption" varStatus="status3">
                                  <tr>
                                    <td width="7%"><input type="radio" name="userAnswer${status.index}" id="userAnswer${status.index}" value="${answerOption.code}" 
                                     <c:if test="${currentPage.userAnswer[status.index]==answerOption.code}">
										checked
									 </c:if>
                                    />
                                    <c:if test="${status3.index==0}">A</c:if>
                                    <c:if test="${status3.index==1}">B</c:if></td>
                                    <td width="93%">${answerOption.content}
