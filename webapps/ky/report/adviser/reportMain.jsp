...
<script type="text/javascript" src="<%=request.getContextPath()%>/js/js4cnltreemenu.js"></script>
...
<div id="main" class="wm4">
	<div class="CNLTreeMenu" id="CNLTreeMenu1">
      <div style="padding-left:10px;"><a href="ad_stu_report.jhtml?userId=${userId}" target="main" id="nodeInstanceId0" <c:if test="${nodeInstanceId==0}"> class="f16px on_none" </c:if><c:if test="${nodeInstanceId>0}">  class="f16px fB"  </c:if> >${userDataVO.processName}</a></div>
    	...
    	<c:choose> 
    		<c:when test="${item.nodeType eq 'GROUP'}"> 
    				<c:if test="${i==0}"></ul></li></c:if>
     				...
     				<li><a id="${item.nodeDefinitionId}" href="ad_stu_report.jhtml?nodeId=${item.nodeInstanceId}&userId=${userId}" target="main"  onclick="clickNode('nodeInstanceId${item.nodeInstanceId}');"><span id="nodeInstanceId${item.nodeInstanceId}" <c:if test="${nodeInstanceId==item.nodeInstanceId}"> class="on_none" </c:if>>${item.nodeName}</span></a><ul class="Child">
	   		</c:when> 
    		<c:otherwise>
     				<li class="Child">     				
     				<a href="ad_stu_report.jhtml?nodeId=${item.nodeInstanceId}&userId=${userId}" target="main"  
...
ExCls(document.getElementById('${nodeGroupId}'),'Opened','Closed',1);
	...
	<div class="nTab wm5">
    <iframe width="795" name="main" id="frame_content" src="<%=request.getContextPath()%>/report/ad_stu_report.jhtml?nodeId=${nodeId}&userId=${userId}" scrolling="no" frameborder="0" onload="this.height=100"></iframe>
		</div>
		<script type="text/javascript">
  ...
  <table width="500" height="200" border="0" cellspacing="0">
    <tr> 
      <td align="center" class="comm" style="line-height:40px;">
        ...
        <p><font color="#CC3300"> 
          <input type="button" value=" 继续训练 " onclick="dosubmit();" id="submitButton"/> <input type="button" value=" 取 消 " onclick="cancel();" />
          </font>
        </p>
      </td>
    </tr>
  ...
  <table width="500" height="200" border="0" cellspacing="0">
    <tr> 
      <td align="center" class="comm" style="line-height:40px;">
        ...
        <p><font color="#CC3300"> 
          <input type="button" value=" 好，开始生词训练 " onclick="location.href='../freshword/freshWordFace.jhtml';" id="submitButton"/> <input type="button" value=" 暂不训练 " onclick="cancel();" />
          </font>
        </p>
      </td>
    </tr>
...
<script type="text/javascript">	
	...
	function cancel(){
		doit(sign01);
		doit(sign02);
		doit(window.frames['frame_content'].document.getElementById('selectType'));
	}
