...
<div class="TabTitle" id="ceshi">
    <ul>
      <li class="normal_2" ><a class="cBlack" href="myAttention.jhtml">我关注的试题</a></li>
      <li class="active_2" ><a class="cBlack" href="topAttention.jhtml">最热关注Top500</a></li>
    </ul>
  </div>
  <!-- 内容开始 -->
  <div class="TabContent">
    ...
    <!--stars tan div-->
		...
		<div class="btn">
		<span class="bbs1"><button class="bb1" onmouseout="this.className='bb1';" onmouseover="this.className='bb2';" onClick="javascript:doit(sign01);doit(sign02)">确认</button></span>
		<span class="bbs1">
...
<div class="content_right wm220 left">
	...
	<script type="text/javascript">
		...
		var d = new dTree('d');
		d.add(0,-1,'<c:if test="${fn:length(userDataVO.processName)>18}"><c:out value="${fn:substring(userDataVO.processName,0,15)}"/>...</c:if><c:if test="${fn:length(userDataVO.processName)<=18}">${userDataVO.processName}</c:if>','topAttention!list.jhtml','${userDataVO.processName}','main');
		<c:forEach items="${mapList}" var="item" > 
			<c:choose> 
				<c:when test="${item['node_group_id'] eq null}"> 
					d.add(${item['id']},0,'${item['name']}','topAttention!list.jhtml?orderNum=${item['order_num']}','${item['name']}','main');
				</c:when>
				<c:when test="${item['node_group_id'] != null}"> 
					d.add(${item['id']},${item['node_group_id']},'${item['name']}','topAttention!list.jhtml?orderNum=${item['order_num']}','${item['name']}','main');
				</c:when>
			</c:choose>
		</c:forEach>
		document.write(d);
        ...
        <iframe width="690" name="main" id="frame_content" src="topAttention!list.jhtml?orderNum=${orderNum}&order=${order}&type=${type}&tag=${tag}&page=${page}" 
