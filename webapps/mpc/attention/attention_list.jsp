...
function goto(order,page){
    var param = "myAttention!list.jhtml?order="+order;
	if(page != null && page !=''){
		param = "myAttention!list.jhtml?page="+page;
	}
...
function send(orderNum,type){
	var href = "myAttention!list.jhtml";
...
$(function(){
	$("input[t=all]").click(function(){
		$("input[t=son]").attr("checked",$(this).attr("checked")); 
	})
	$("#tt").click(function(){
		var str = "";
		$("input[t=son]").each(function(){
			if($(this).attr("checked"))
			str += $(this).val()+","; 
		})	
		window.parent.attention(str);			
	})
})
...
function showDiv(divId){	
		...
		return slideUp();
            ...
            function slideUp(){
                $("#divObj").slideUp("slow");
				viewObj.style.background="url(images/view+.gif) no-repeat";
            }
...
<body >
        ...
        <div class="tager">
        <div class="tager_in">
          <h1 class="f14px">你共关注了${attentionAmount}道试题，记了${noteAmount}条笔记。</h1>
          ...
          <h2><span class="f14px cblue fB">我用过的标签：</span>
		  <c:forEach items="${tagList}" var="tagObj" >
		  <span>
		  <c:if test="${tag == tagObj['id']}">
		  <a href="myAttention!list.jhtml?orderNum=${orderNum}&type=${type}&tag=${tagObj['id']}" style="color:gray;font-weight:bold">${tagObj['tag']}</a>
		  </c:if>
		  <c:if test="${tag != tagObj['id']}">
		  <a href="myAttention!list.jhtml?orderNum=${orderNum}&type=${type}&tag=${tagObj['id']}" >${tagObj['tag']}</a>
		  </c:if>
		  </span> 
		  </c:forEach>
		   </h2>
		   <div id="divObj" style="overflow:hidden; display:none;">
            <h2>		  
			<c:forEach items="${moreTagList}" var="tagObj" >
		  <span>
		  <c:if test="${tag == tagObj['id']}">
		  <a href="myAttention!list.jhtml?orderNum=${orderNum}&type=${type}&tag=${tagObj['id']}" style="color:gray;font-weight:bold">${tagObj['tag']}</a>
		  </c:if>
		  ...
		  <a href="myAttention!list.jhtml?orderNum=${orderNum}&type=${type}&tag=${tagObj['id']}" >${tagObj['tag']}</a>
			...
			<c:if test="${fn:length(tagList)>=50}">
			<div style="margin-left:520px;">
			<span id="viewObj" style="background:url(images/view+.gif) no-repeat; padding-left:20px; margin-left:20px; line-height:16px;"><a href="javascript:void(null)" onclick="showDiv()">更多标签</a></span>
          </div>
		    </c:if>
        </div></div>
                ...
                <ul>
                    <li class="ToolbarItem c_mcp BigPaddingLeft" id="sortMenu">
                        ...
                        <ul style="display: none; visibility: hidden; left: 33px; top: 28px;" class="c_m" onclick="$menu.closeCurrent();">
                             ...
                             <li aid="SortBySender">
							 	<c:if test="${order != null  && order !='' && order != 'time_asc' && order != 'time_desc'}">
                                <a id="SortBySender" href="#" title="关注时间" onclick="javascript:goto('time_desc');">				                                <span class="c_light">关注时间</span>
								</a>
								</c:if>
							 	<c:if test="${order == 'time_asc'}">
                                <a class="sel" id="SortBySender" href="#" title="关注时间" onclick="javascript:goto('time_desc');">				                                <span class="c_light">关注时间</span>
								<img src="../images/clear.gif" class="ascend_rest_dark" alt="正向排序">
								</a>
								</c:if>
								<c:if test="${order == null || order =='' || order == 'time_desc'}">
                                <a class="sel" id="SortBySender" href="#" title="关注时间" onclick="javascript:goto('time_asc');">				                                <span class="c_light">关注时间</span>
                             ...
                             <li aid="SortBySender">
							 	<c:if test="${order != 'attention_asc' && order != 'attention_desc'}">
								...
								<c:if test="${order == 'attention_desc'}">
...
<script defer="defer" src="../js/shared.js"></script>
            ...
            <td width="12%" align="center">受关注人气</td>
            <td width="12%" align="center">共享笔记总数</td>
            <td width="10%" align="center">精华笔记</td>
            <td width="10%" align="center">我的笔记</td>
            <td width="11%" align="center">题型</td>
            <td width="6%" align="center">难度</td>
            </tr>
			<c:forEach items="${attentionList}" var="attention" > 
          <tr>
            <td width="6%"><input  type="checkbox" value="${attention['id']}"  t=son /></td>
            <td width="23%" style="margin-left:auto; margin-right:auto;width:200px;">
            	<a href="#" onclick="javascript:parent.location.href='showAttention.jhtml?attentionId=${attention.id}&type=my'" >${attention['content']}</a>
            </td>
            <td align="center">
				<fmt:formatDate value="${attention['insert_time']}" pattern="yyyy-MM-dd"/>
			</td>
        ...
        <div class="list_gz_inp"><span class="inp">
            <input type="checkbox" t=all />
            全选</span><span class="inp">
            <input type="submit"  name="tt" id="tt" value="不再关注" />
            </span></div>
        <div class="blank6"></div>
