...
dTree.prototype.node = function(node, nodeId) {
	...
	if (node.url) {
		if(this.root.id == node.pid){
			str += '<a id="s' + this.obj + nodeId + '" class="' + ((this.config.useSelection) ? ((node._is ? 'rootNodeSel' : 'rootNode')) : 'rootNode') + '" href="' + node.url + '"';
...
dTree.prototype.gInd=function(id){
	var n=0;
  	for ( n=0; n<this.aNodes.length; n++) {
  		if(this.aNodes[n].id==id){ break; }
	}
	return n;
};