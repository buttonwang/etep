		        		...
		        		<c:forEach items="${item.answerOptions}" var="answerOption" varStatus="answerStatus">
							...
							<li>
								<input type="radio" name="userAnswer${status.index}" id="userAnswer${status.index}" value="${answerOption.code}"
									<c:if test="${item.examProperty.userAnswer==answerOption.code}">checked</c:if>
								/>${answerCode} ${answerOption.content}							 	
							 	<c:if test="${viewControl.showModel!=1}">
									<c:if test="${item.examProperty.isRight&&answerOption.code==item.examProperty.userAnswer}">
