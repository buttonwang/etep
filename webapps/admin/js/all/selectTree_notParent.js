...
function SelectOrDivTree_printOption(obj,str,selectedId,outStr,prefix,theId,withParentName,parentName){
	if(obj.id!=theId){
		var withParentName = withParentName||false;
		var str  = str||"" ;
		var thisSonsShowStr ="";
		obj._selected =  (selectedId||"")==obj.id?"selected":"";
		obj._prefix=str||"";
		obj.__parentName = parentName||"";
		var sons =obj.sons||[];	
		if(withParentName&&sons.length==0){
			outStr[outStr.length]=[SelectOrDivTree_LparamObj("<option value={obj.id} {_selected}>{_prefix}{__parentName}/{obj.name}</option>",obj)].join("") ; 
		}else{
			outStr[outStr.length]=[SelectOrDivTree_LparamObj("<option value={obj.id} {_selected}>{_prefix}{obj.name}</option>",obj)].join("") ;  
		}		
		str +=prefix;
		for(var p in sons){
			SelectOrDivTree_printOption(sons[p],str,selectedId,outStr,prefix,theId,withParentName,obj.obj.name||"" );
		}
	}
}
function SelectOrDivTree(data,selectedId,outArr,prefix,getStr_f,theId,withParentName){
	var prefix = prefix||"--";
	...
	for(var i in noParentData){
		...
		SelectOrDivTree_printOption(obj,"",selectedId,out,prefix,theId,withParentName);
	}
	return out.join("");
}
...
function selectTree(map){
	...
	var withParentName = map.withParentName||false;
	...
	optionStr += SelectOrDivTree( dataLst,selected,[],prefix,null,theId,withParentName);	 
