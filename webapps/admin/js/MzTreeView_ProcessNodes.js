...
var MzTreeViewTD="</td><td class='MzTreeViewCell0 wtd2'>{orderNum}</td><td class='MzTreeViewCell0  wtd3'>{type}</td></td><td class='MzTreeViewCell1 wtd4'>&nbsp;&nbsp;<a href='node.jhtml?atype=edit&to=showProcess&id={id}&processId={prid}&prid={prid}'>修改</a>&nbsp;&nbsp;<a href='node.jhtml?atype=sadd&p.para.pid={id}&p.para.prid={prid}&p.para.nid={id}&prid={prid}'>增加</a>&nbsp;&nbsp;<a href='node.jhtml?atype=delete&id={id}&p.para.prid={prid}'>删除</a></td></tr></table>";	

function MzTreeView(Tname,nodeForMz,getN_F,isShowCheckBox)
{
  ...
  this.N = {};  
  if(getN_F!=undefined){
	   this.N =getN_F(nodeForMz);
  }
  this.isShowChecBox = isShowCheckBox||false;
  this.node  = {};
  this.names = "";
  this._d    = "\x0f";
  this.index = 0;
  ...
  var nodeForMz =nodeForMz||[];
  var getIdObject=function (nodeForMz,divider){
		var data = [];
		 
		for(var i in nodeForMz){
			var sonId =nodeForMz[i].pid_id.substr(nodeForMz[i].pid_id.indexOf(divider) + divider.length);
			data[sonId] = nodeForMz[i].obj; 
		}
		return data;	
  }
  this.data =getIdObject(nodeForMz,this.divider);  
  this.node["0"] =
  {
    "id": "0",
    "path": "0",
...
MzTreeView.prototype.nodeToHTML = function(node, AtEnd)
{
  ...
  try {
    ...
    if ( typeof MzTreeViewTD != "undefined" ) HTMTD=this.paramObj(MzTreeViewTD,_data_obj);
  } catch(e){}
...
MzTreeView.prototype.paramObj=function(){
